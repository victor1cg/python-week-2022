...
class Beer(SQLModel, table = True) :                                    #Pelo fato de ser SQLModel, é possivel criar tabelas SQL
    ...
    @validator("flavor","image","cost")                                  #DECORATOR - função associada a classe, é metodo.
    def validating_ratings(cls, v, field):                               #cls - classe, v - valor a ser validado, field - campo a ser validado
        if v < 1 or v > 10:                                              #função associada a essa classe.
            raise RuntimeError(f'\n{field.name.upper()} must be between one and ten!')
        return v

    @validator('rate', always = True)                                    #@validator vai acionar e retornar a função
